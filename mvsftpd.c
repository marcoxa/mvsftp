...
/*
 * Made more "Emacs-friendly" (I know; don't ask!)
 * Marco Antoniotti, 2021.
 *
 * Also reformatted.  Don't we want 80 columns? (Maybe set fill column to 72).
 *
 */
...
/* #define debug_output */ /* Define to get debug output. */
...
static void * safe_malloc(size_t s) {
	...
	printf("FTP016E memory allocation failed.\n");
	exit(16);
	return (void *) 0;	/* Useless but saves a warning in JCC. */
    }
}
...
/* MAXBUFLEN
 * Normal buffer size for FTP - all/most NW cards/drivers support
 * this.
 */
#define MAXBUFLEN 4096 
 
/* Largest buffer size allowed in Windows */
/* #define MAXBUFLEN 32768 */
 
/* used for socket send/recv max size, buffers are twice as big (plus
 * one for an optional terminator.)
 */
...
#define LOGIN_MESSAGE_C \
    "FTP001I %s - logged in - TIME=%.2d.%.2d.%.2d DATE=%d/%.2d/%.2d"
 
#define REJECT_MESSAGE \
    "FTP002I %d.%d.%d.%d - rejected - %.2d.%.2d.%.2d %d/%.2d/%.2d"
 
#define LOGOUT_MESSAGE_C \
    "FTP003I %s - logged out - TIME=%.2d.%.2d.%.2d DATE=%d/%.2d/%.2d"
...
/**** The following is a back-up mechanism for OSs like Windows that
 * don't return information in getsockname.
 */
char  PASVADDR[16]; /* PASV style string for hosts that don't return
		     * their IP Address.
		     */
 
static long running = 1;  /* We are running */
 
static long optimise = 0; /* The largest numbered socket descriptor,
...
typedef struct cb_data_tag {
    ...
    void* (*READ_FUNC) (void *, void *);  /* function address to call
					   * for reads
					   */
    void* (*WRITE_FUNC) (void *, void *); /* function address to call
					   * for writes
					   */
...
typedef struct data_tag {
    long   use_count;  /* May have many references, so only delete the
			* structure when no more exist.
			*/
    SOCKET SOCK;    /* The MASTER-SOCKET which this data belongs to. */
    long   DONE;    /* Signal that the MASTER-SOCKET will be used no more. */
 
    char * READ_BUF;		/* Incoming commands buffer. */
    ...
    struct sockaddr * DATA_CONNECT_ADDR; /* malloc'd area for building
					  * connect to client-socket
					  * information.
					  */ 
    SOCKET DATA_SOCK; /* All file related data transfers uses this SOCKET. */
    SOCKET DATA_LISTEN_SOCK; /* PASV mode transfers allow connecting
			      * to this SOCKET.
			      */
    
    FILE * FILE; /* File transfers read/write to this descriptor. */
    char   DATA_SOCK_STATUS; /* A-Active, C-Connected, F-Failed,
			      * L-Listening.
			      */
    ...
    char * CWD; /* This users current working directory (ie. root:'\'
		 * or PDS:'\...\' in MVS)
		 */
} data_t;
 ...
 * values are (refs to) : filehandle, data, read_func, write_func
 *                       (could add accept_func)
 ...
 * vec
 */
static long vec (fd_set * bits, SOCKET fd, long value) {
    /* value = -1 just returns the current status */
...
*/
static void set_callbacks_f (FILE * fh,
			     void * func_data,
			     void * read_func,
			     void * write_func) {
    data_tag_ptr d;
 ...
 * set_callbacks
 */
static void set_callbacks (SOCKET fh,
			   void * func_data,
			   void * read_func,
			   void * write_func) {
    data_tag_ptr d;
...
static void run_callbacks () {
	...
	/* First, check file-processing to-do: */
        while (cb_data) {
	...
	/* Done checking file-processing to-do. */
        ...
        if (i > optimise) i = optimise; /* may be greater than the
					 * known value.
					 */
        ...
        if (i < 0) i = 0; /* Algorithm fix for IBM sockets (descr 0
			   * allowed).
			   */
	...
	printf ("select entered c=%d r=%08X w=%08X\n",
        ...
        if (__libc_osvm == 0) { /* z/OS/390 or MVS3.8j?  No - Don't
				 * scan CIB chain
				 */
	    /* Wait - No timelimit. */
            i = select (i + 1, &rout, &wout, NULL, NULL);
 
        } else {
 
            j = 0;
            while ((j == 0) && (running)) {
		/* Wait - With a timelimit. */
                j = select (i + 1, &rout, &wout, NULL, &t);
 
                if (j == 0) { /* Scan the CIB chain for a STOP command */
                    s = (*((long **)548));       /* 548->ASCB */
                    s = ((long **)s) [14];       /*  56->CSCB */
                    if (s) {
                        if (__libc_oarch) { /* z/OS/390 uses an
					     * extension
					     */
                        ...
                        while (s) {
                            if (((unsigned char *)s) [4] == 0x40) {
                                running = 0; /* Found a STOP command
					      * queued.
					      */
                                break;
                            }
                    ...
                    if (running) { /* no STOP command found,
				    * reinitialize arrays
				    */
                        rout = rbits; /* Never modify the real ones... */
                        wout = wbits; /* 'select' always returns a new
				       * array.
				       */ 
                    }
                }
	 ...
	 */
        ...
        while (cb_data) {
            ...
            fd = cb_data->FH;
            cb_next = cb_data->next; /* In case it's deleted
				      * below...
				      */
 
            if (fd != -1) { /* Sockets only */
 
                /* If this fd is ready for reading/writing and the
		 * callback exists.
		 */
                if ((vec (&rout, fd, -1)) && (cb_data->READ_FUNC)) {
		    /* Call it */
                    (*cb_data->READ_FUNC) ((void *)fd, cb_data->FUNC_DATA);
 
                } else /* Can't have both readers and writers */
		    if ((vec(&wout, fd, -1)) && (cb_data->WRITE_FUNC)) {
			/* Call it */
			(*cb_data->WRITE_FUNC) ((void *)fd,
						cb_data->FUNC_DATA);
		    }
            }
...
/**
 * grab_line
 * Reads a line (up to LF) from the beginning of a string, and returns
 * it the line is removed from the string (returns NULL for a line
 * without a linefeed.)
 */
...
/**
 * generic_write
 * write data to the buffer for the given filehandle, and wake up the
 * callback for it
...
/**
 * generic_write_line
 * write a line to the buffer for a given filehandle, and wake
 * up the callback for it.
 * (takes: a filehandle, a buffer, and data to write)
 */
...
static void ctl_write_line (data_tag_ptr data, char * line) {
    ...
    generic_write_line (data->SOCK,
			&(data->WRITE_BUF [data->WRITE_BUF_LENGTH]),
			line);
...
static char * sockaddr_to_comma_list (SOCKET SOCK) {
    ...
    if (b->sin_addr.s_addr) {
	 /* *** NOT ALL IMPLEMENTATIONS EXPORT THE HOST IP-ADDRESS, SO
	  * USE PASVADDR ***
	  */
...
/**
 * ftp_server_msg
 * takes a server response code (like 230)
 * and a ftp message (which may be several lines, and may not be empty)
...
static data_tag_ptr new_ftp_data (SOCKET SOCK) {
    ...
    data->DATA_LISTEN_SOCK = -1;   /* The socket to wait for a
				    * connection on, in PASV mode
				    */
    data->FILE = NULL;             /* The file to read from/write to */
 
    data->DATA_SOCK_STATUS = ' '; /* Either " " (none),
				   * "L" (listening),
				   * "C" (connecting),
				   * "F" failed,
				   * "A" (active)
...
static void file_wcb (FILE * file, data_tag_ptr data) {
    ...
    if (data->DATA_SOCK_BUF_LEN) {
        ...
        if (data->TYPE == 'A') {
            ...
            if (t [data->DATA_SOCK_BUF_LEN - 1] == acrlf [0]) {
                data->DATA_SOCK_BUF_LEN--; /* remove the split control
					    * character
					    */
            }
            ...
            while (s) {
                s [0] = acrlf [1]; /* Make CR - LF, but don't write
				    * the second one
				    */
            ...
            status += write_from_buf_f (file, t, strlen (t)); /* embedded nulls
							       * not allowed!
							       */
 
        } else {
 
            status = write_from_buf_f (file,
				       data->DATA_SOCK_BUF,
				       data->DATA_SOCK_BUF_LEN);
    ...
    if (data->DATA_SOCK == -1) {
        /* If there's no socket open to get more data from, the
	 * transfer is done
	 */
        reset_data_connection (data);
    } else {
...
static void file_rcb (FILE * file, data_tag_ptr data) {
    ...
    status
	= read_into_buf_f (file,
			   &(data->DATA_SOCK_BUF [data->DATA_SOCK_BUF_LEN]));
...
static void data_sock_wcb (SOCKET data_sock, data_tag_ptr data) {
    ...
    if (data->DATA_SOCK_BUF_LEN) {
        status = write_from_buf (data_sock, data->DATA_SOCK_BUF,
				 data->DATA_SOCK_BUF_LEN);
 
        if (status <= 0) {
            printf ("error writing to data connection!\n");
            sprintf (tstr, "425 Data connection failed: %d",
		     HercGetLastError ());
            ctl_write_line (data, tstr);
            reset_data_connection (data);
            return;
    ...
    if (data->DATA_SOCK_BUF_LEN == 0) {
        if (data->FILE == NULL) {
            /* If there's no file open to get more data from,
...
static void data_sock_rcb (SOCKET data_sock, data_tag_ptr data) {
    ...
    status = read_into_buf (data_sock,
			    &(data->DATA_SOCK_BUF [data->DATA_SOCK_BUF_LEN]));
    ...
    /* If there's an EOF (or an error, for now !!!), signal that the
...
static void set_xfer_callback (data_tag_ptr data) {
    ...
    if (data->DATA_SOCK_STATUS == 'A') {
	     ...
	     * and the callback will be put in once the connection is
	     * made
	     */
            remove_callbacks (data->DATA_SOCK);
        }
...
static void prepare_data_sock (data_tag_ptr data) {
    ...
    if (data->DATA_SOCK_STATUS == 'C') {
        data_conn_status = check_sock_connect (data->DATA_SOCK,
					       data->DATA_CONNECT_ADDR);
    } else {
        if (data->DATA_SOCK != -1) {
            data_conn_status = 1;
        } else {
    ...
    if (!data_conn_status) {
        ...
        if (data->XFER) {
            sprintf (tstr, "425 Can't open data connection: %d",
		     HercGetLastError ());
            generic_write_line (data->SOCK,
				&(data->WRITE_BUF [data->WRITE_BUF_LENGTH]),
				tstr);
...
/**
 * data_connect_sock_wcb
 * waits for DATA_SOCK to connect
 */
static void data_connect_sock_wcb (SOCKET data_connect_sock,
				   data_tag_ptr data) {
...
static void ctl_write_msg (data_tag_ptr data, long code, char * message) {
    ...
    generic_write (data->SOCK, &(data->WRITE_BUF [data->WRITE_BUF_LENGTH]),
		   to_write);
    data->WRITE_BUF_LENGTH = strlen (data->WRITE_BUF);
...
static void refresh () {
    ...
    numdevs = 0;
    i = dirsupport (); /* Re-read the VATLST00 file (not really
			* needed)
			*/
    if (i == 0) {
 ...
 * mystrcat
 */
static char * mystrcat (char * dest, char * src, long * clen, long * j) {
    /* j is always the 'end' of dest */
    long i;
...
static char * ls_dash_l (char * target, unsigned int acee) {
    ...
    if (strcmp (target, "/") == 0) { /* Root */
        ...
        if (added_ddc) { /* If special DDs are in use, set up a dummy
			  * date
			  */
            ...
            i = 0;
            while (i < added_ddc) { /* Add the user DD list to the
				     * output
				     */
                size += stat[1];
                type = '-';
	...
	/* Comment-out for faster directory refreshes (using the cache).
         * Then, to update the listing (if required) use the "REFR"
         * command or ftpcmd.c
	 */
...
static void ftp_sock_rcb (SOCKET sock, data_tag_ptr data) {
    ...
    printf ("Recv %d bytes: %s\n",
	    status,
	    &(data->READ_BUF [data->READ_BUF_LENGTH]));
#endif
	...
	printf ("Processing line: %s\n", line);
		 ...
		 */
                ...
                i = strlen (line) + 2;
                memmove (&(data->READ_BUF [i]),
			 data->READ_BUF,
			 data->READ_BUF_LENGTH);
                ...
                if (data->ACEE > 1) {
                    ...
                    time (&lt);
                    td = localtime (&lt);
                    sprintf (wtomsg, LOGIN_MESSAGE_C,
			     data->USER,
			     (* td).tm_hour,
			     (* td).tm_min,
			     (* td).tm_sec,
                ...
                if ((args [0] == 'F') || (args [0] == 'f')) {
                    ...
                    ctl_write_line (data, "504 Unimplemented STRU type");
                }
                ...
                if (data->DATA_LISTEN_SOCK == -1) {
                    ...
                    set_callbacks (data->DATA_LISTEN_SOCK,
				   data,
				   &data_listen_sock_rcb,
				   NULL);
 
                    comma_list =
			sockaddr_to_comma_list (data->DATA_LISTEN_SOCK);
                    sprintf (tstr, "227 Entering Passive Mode (%s)",
			     comma_list);
                    ctl_write_line (data, tstr);
                    free (comma_list);
                }
		...
		/* ****
		   ...
		   if (data->DATA_SOCK == -1) {
		   sprintf (tstr, "500 PORT command unsuccessful cd: %d",
		   i);
		   ctl_write_line (data, tstr);
		   } else {
		   data->DATA_SOCK_STATUS = 'C';
		   set_callbacks (data->DATA_SOCK,
		   data,
		   NULL,
		   &data_connect_sock_wcb);
		   ...
		   */
                ...
                if (prepare_for_xfer (data)) {
                    ...
                    if (data->DATA_SOCK_BUF_LEN) {
                        ...
                        printf ("Directory: %s\n", data->DATA_SOCK_BUF);
#endif
                        ebcdic2ascii (data->DATA_SOCK_BUF,
				      data->DATA_SOCK_BUF_LEN);
                    }
	    ...
	    } else if (strcmp (cmd, "XPWD") == 0) { /* 20210425: MA added. */
                sprintf (tstr, "257 \"%s\" is current directory.", data->CWD);
                ctl_write_line (data,  tstr);
		...
		/* Should not need a dir-refresh on this one */
                if (ftp_chdir (data, "..")) {
                ...
                if (real_path [6] != 0) {
                    ...
                    if (s == NULL) { /* open root file */
                        ...
                        j = 0; /* Check if the name matches a known
				* user DD
				*/
                        while (j < added_ddc) {
...
#ifdef debug_output
                    if (real_path [0] != 0)
			printf ("Opening filename: %s (Read)\n", real_path);
#endif
...
#ifdef debug_output
                    if (real_path [0] != 0)
			printf ("Opening filename: %s (Write)\n", real_path);
#endif
                    ...
                    if (real_path [0] == 0) {
                        ...
                        if (fh == NULL) {
                            ...
                            sprintf (tstr, "550 %s: Not opened %d",
				     args,
				     errno);
                            ctl_write_line (data, tstr);
		...
		if (real_path [6] != 0)  {
				...
				*/
				...
				/* Try again now */
                            ...
                            if (org != MVS_DSORG_PS) /* First check it's PS */
                                real_path [0] = 0;
                        }
                    } else {         /* Open pds file */
                        s [0] = '(';
			...
			printf ("Opening filename: %s (Read)\n", real_path);
			     ...
			     *
			     * Return the current time FTTB just to
			     * check that the logic is ok.
			     */
			    char fmtdate [80];
			    
			    time (&lt);
			    td = localtime(&lt);
			    strftime (fmtdate, 80, "2%Y%m%d%H%M%S", td);
			    /* People can worry in the year 3000... */
			    
			    sprint (tstr, "213 %s", fmtdate);
			    ctl_write_line (data, tstr);
                        }
                    }
                } else {
...
static void ftp_sock_wcb (SOCKET sock, data_tag_ptr data) {
    ...
    bytes_written = write_from_buf (sock,
				    data->WRITE_BUF,
    ...
    if (serv_sock < 0) {
        ...
        while (head) {
            ...
            while (data) {
                ...
                if (ohead == head)
                    head = head->next; /* Something bad happened,
					* ignore this entry.
					*/
                break;
            }
