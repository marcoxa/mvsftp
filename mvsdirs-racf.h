...
#ifndef _MVSDIRS_RACF_H
#define _MVSDIRS_RACF_H
...
static long dirsupport () {
    ...
    /* There is no member VATLSTFF in vanilla TK4- */
...
#define ucbnchr(dest, scan) { \
	memcpy (dest, &((scan)->parm [36]), 4); \
	dest [4] = 0; \
    }
...
#define MVS_DSORG_UNKNOWN 0
#define MVS_DSORG_PS 1
#define MVS_DSORG_PO 2
...
typedef struct dirs_tag {
    char name [45];
    long date;
    long org;   /* 1 = PS, 2 = PO */ /* See above: MA 20210516 */
} dirs;
...
static long readvtoc (char * device, char * unit, p_root rt) {
    ...
    while (fread (b, 1, 96, fh) == 96) {
        ...
        if (b [82] == 0x40)
            c->directories [c->dircount].org = MVS_DSORG_PS;
        else
            c->directories [c->dircount].org = MVS_DSORG_PO;
        ...
        c->directories [c->dircount].date
	    = i * 24 * 60 * 60; /* time = seconds past 1970 */
...
#endif /* _MVSDIRS_RACF_H */

/* end of file -- mvsdirs-racf.h */
